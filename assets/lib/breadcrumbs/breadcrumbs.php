...
class Breadcrumb_Trail {
        ...
        if ( 0 < $item_count ) {
            ...
            foreach ( $this->items as $item ) {
                ...
                $item = ! empty( $matches )
                    ...
                    : sprintf( '<a href="'.esc_url( $link_item ).'" itemprop="item" aria-label="%s">%s</a>', $item );
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'masonry-grid' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'masonry-grid' ),
            'home'                => esc_html__( 'Home',                                  'masonry-grid' ),
            'error_404'           => esc_html__( '404 Not Found',                         'masonry-grid' ),
            'archives'            => esc_html__( 'Archives',                              'masonry-grid' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'masonry-grid' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'masonry-grid' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'masonry-grid' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'masonry-grid' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'masonry-grid' ),
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'masonry-grid' ) ) );
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'masonry-grid' ) ) );
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'masonry-grid' ) ) );
    }
    ...
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'masonry-grid' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'masonry-grid' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'masonry-grid' ) ) );

        // Add the year and month items.
        $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y' ) ) ), $year );
        $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) ), $month );

        // Add the day item.
        if ( is_paged() )
            $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y' ) ), get_the_time( 'm' ), get_the_time( 'd' ) ), $day );
    ...
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'masonry-grid' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'masonry-grid' ) ) );
    ...
    protected function add_search_items() {
        ...
        if ( is_paged() )
            $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_search_link() ), sprintf( $this->labels['search'], get_search_query() ) );
    ...
    protected function add_post_parents( $post_id ) {
        ...
        while ( $post_id ) {
            ...
            $parents[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_permalink( $post_id ) ), get_the_title( $post_id ) );
    ...
    protected function add_post_hierarchy( $post_id ) {
        ...
        if ( $post_type_object->has_archive ) {
            ...
            $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_post_type_archive_link( $post_type ) ), $label );
        }
    ...
    protected function add_post_terms( $post_id, $taxonomy ) {
        ...
        if ( $terms && ! is_wp_error( $terms ) ) {
            ...
            $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), $term->name );
        }
    }
    ...
    function add_term_parents( $term_id, $taxonomy ) {
        ...
        while ( $term_id ) {
            ...
            $parents[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), $term->name );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                    $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'masonry-grid' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag )
                    $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'masonry-grid' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag )
                    $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'masonry-grid' ) ) ) );

                // If using the %author% tag, add a link to the post author archive.
                elseif ( '%author%' == $tag )
                    $this->items[] = sprintf( '<a href="%s" aria-label="%s">%s</a>', esc_url( get_author_posts_url( $post->post_author ) ), get_the_author_meta( 'display_name', $post->post_author ) );
