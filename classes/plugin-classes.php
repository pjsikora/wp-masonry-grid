<?php
if ( !class_exists('Masonry_Grid_Getting_started') ):

    class Masonry_Grid_Getting_started
    {   

        function __construct()
        {	

	        add_action( 'wp_ajax_masonry_grid_install_plugins', array( $this, 'masonry_grid_install_plugins' ) );
        ...
        public static function masonry_grid_plugin_status($plugin_class, $plugin_folder, $plugin_file){

            if( class_exists( $plugin_class ) ){

                return array('status' => 'active','string' => esc_html__('Deactivate','masonry-grid') );
                ...
                if( file_exists( $path ) ) {
                    ...
                    return array('status' => 'deactivate','string' => esc_html__('Activate','masonry-grid') );
                }else{

                    return array('status' => 'not-install','string' => esc_html__('Install & Active','masonry-grid') );
        ...
        public static function masonry_grid_recommended_plugins(){
        ...
        public function masonry_grid_install_plugins(){
            
            $nonce = isset( $_POST["_wpnonce"] ) ? sanitize_text_field( wp_unslash( $_POST["_wpnonce"] ) ) : '';

            if ( ! current_user_can('install_plugins') ) {
                wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'masonry-grid' ) );
            }

            // Check our nonce, if they don't match then bounce!
            if (! wp_verify_nonce( $nonce, 'masonry_grid_ajax_nonce' )) {

                wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'masonry-grid') );
            ...
            if( isset( $_POST['single'] ) ){
                ...
                $plugin_lists = $this->masonry_grid_recommended_plugins();
            ...
            foreach( $plugin_lists as $key => $plugin ){
                ...
                if( isset( $_POST['single'] ) ){
                    ...
                    $pluginstatus   = $this->masonry_grid_plugin_status( $plugin['class'],$key,$plugin['PluginFile'] );
                ...
                if( isset( $_POST['single'] ) && $PluginStatus == 'active' ) {
                    ...
                    esc_html_e('Deactivated' , 'masonry-grid');
                    die();
                    ...
                    if( isset( $_POST['single'] ) ) {
                        ...
                        esc_html_e('Activated' , 'masonry-grid');
                        die();
                    ...
                    if( file_exists($plugin_dir) ) {
                        ...
                        if( isset( $_POST['single'] ) ) {
                            esc_html_e('Installed & Activated' , 'masonry-grid');
                            die();
                        }
                    ...
                    if( isset( $_POST['single'] ) ) {
                        ...
                        esc_html_e('Failed' , 'masonry-grid');
                        die();
            ...
            if( !isset( $_POST['single'] ) ) {
                echo esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=masonry-grid-about' );
            }
    ...
    new Masonry_Grid_Getting_started();
