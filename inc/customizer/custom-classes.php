...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    class Masonry_Grid_Custom_Radio_Image_Control extends WP_Customize_Control {
...
// Radio COlor Schema
class Masonry_Grid_Custom_Radio_Color_Schema extends WP_Customize_Control {
 ...
 * @access public
 */
class Masonry_Grid_Customize_Section_Upsell extends WP_Customize_Section {
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Repeator Info
    class Masonry_Grid_Premium_Notice_Control extends WP_Customize_Control {

        public $type = 'masonry-grid-premiuminfonotice';
            ...
            <span class="customize-control-title">
                <div class="theme-info-icon">
                    <div class="icon-notice-wrap">
                        ...
                        <span><?php echo esc_html__('More ','masonry-grid'). esc_html( $this->label ). esc_html__(' Available on Premium Version.', 'masonry-grid' ); ?></span>
                    </div>
                </div>
            </span>
...
*/
class Masonry_Grid_Repeater_Controler extends WP_Customize_Control {
    /**
     * The control type.
     *
     * @access public
    ...
    */
    ...
    public $masonry_grid_box_label = '';

    public $masonry_grid_box_add_control = '';

    public $masonry_grid_box_add_button = '';
    ...
    public function __construct( $manager, $id, $args = array(), $fields = array() ) {
        $this->fields = $fields;
        $this->masonry_grid_box_label = $args['masonry_grid_box_label'] ;
        $this->masonry_grid_box_add_control = $args['masonry_grid_box_add_control'];
        $this->masonry_grid_box_add_button = $args['masonry_grid_box_add_button'];
    ...
    public function render_content() {
        ...
        <ul class="masonry-grid-repeater-field-control-wrap">
            <?php
            $this->masonry_grid_get_fields();
            ?>
        </ul>
        <input type="hidden" <?php esc_attr( $this->link() ); ?> class="masonry-grid-repeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />
        <?php

        if( isset( $this->masonry_grid_box_add_button ) && $this->masonry_grid_box_add_button ){ ?>
            
            <button type="button" class="button masonry-grid-add-control-field"><?php echo esc_html( $this->masonry_grid_box_add_control ); ?></button>
        <?php
        }
    ...
    private function masonry_grid_get_fields(){
        ...
        ?>
        <li class="masonry-grid-repeater-field-control">

        <div class="title-rep-wrap">
            <span class="dashicons dashicons-move twp-filter-icon"></span>
            <h3 class="masonry-grid-repeater-field-title"><?php echo esc_html( $this->masonry_grid_box_label ); ?></h3>
            <span class="dashicons dashicons-arrow-down twp-filter-icon"></span>
        </div>

        <div class="masonry-grid-repeater-fields">
        <?php
            foreach ($fields as $key => $field) {
            $class = isset($field['class']) ? $field['class'] : '';
            ?>
            <div class="masonry-grid-fields masonry-grid-type-<?php echo esc_attr($field['type']).' '.esc_attr( $class ); ?>">
                <?php 
                    ...
                    switch ($field['type']) {
                        ...
                        case 'selector_color':
                        $options = $field['options'];
                            echo '<div class="masonry-grid-type-radio">';
                        ...
                        case 'upload':
                                ...
                                esc_html_e('No image selected', 'masonry-grid');
                                echo '</div>';
                                echo '<div class="thumbnail thumbnail-image">';
                                echo $image;
                                echo '</div>';
                                echo '<div class="actions clearfix">';
                                echo '<button type="button" class="button theme-image-delete align-left">'.esc_html__('Remove', 'masonry-grid').'</button>';
                                echo '<button type="button" class="button theme-custom-upload-button alignright">'.esc_html__('Select Image', 'masonry-grid').'</button>';
            ...
            <div class="clearfix masonry-grid-repeater-footer">
                <div class="alignright">
                <?php if( isset( $this->masonry_grid_box_add_button ) && $this->masonry_grid_box_add_button ){ ?>
                    <a class="masonry-grid-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'masonry-grid') ?>|</a> 
                <?php } ?>
                <a class="masonry-grid-repeater-field-close" href="#close"><?php esc_html_e('Close', 'masonry-grid') ?></a>
                </div>
            </div>
        </div>
        </li>
...
if ( ! function_exists( 'masonry_grid_sanitize_number_range' ) ) :
     ...
     */
    function masonry_grid_sanitize_number_range( $input, $setting ) {
